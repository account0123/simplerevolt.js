 ...
 */
async function handleEvent(client, event, setReady) {
    ...
    switch (event.type) {
        ...
        case "Ready": {
            for (const user of event.users) {
                ...
                if (u.relationship == __1.Relationship.User) {
                    client.user = u;
                }
            }
        ...
        case "ChannelCreate": {
            if (!client.channels.cache.has(event._id)) {
                client.channels._add(__1.Channel.from(client, event));
            }
            break;
        }
        case "ChannelUpdate": {
        ...
        case "ServerCreate": {
            if (!client.servers.cache.has(event.server._id)) {
                client.servers._add(new __1.Server(client, event.server));
                for (const channel of event.channels) {
                    client.channels._add(new __1.Channel(client, channel));
                }
            }
            break;
        }
