  ...
  if (target instanceof Server) {
    ...
    if (target.ownerId == user?.id) {
      ...
      if (member.roles && target.roles) {
        ...
        const permissions = member.orderedRoles.map(
          (role) => role.permissions || new PermissionOverrides(target, { id: role.id, a: 0, d: 0 }),
        );
    ...
    switch (target.channelType) {
      ...
      case "DirectMessage": {
        ...
        const user_permissions = (target as DMChannel).recipient?.permission || 0;
      ...
      case "VoiceChannel": {
        // 2. Get server.
        const channel = target as ServerChannel;
